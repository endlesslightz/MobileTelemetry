...
function initDatabase() {
  ...
  database.transaction(function(transaction) {
    transaction.executeSql('CREATE TABLE `user` ( `id_user` INT NOT NULL AUTO_INCREMENT , `nama_user` VARCHAR(255) NOT NULL , `username` VARCHAR(255) NOT NULL , `password` VARCHAR(50) NOT NULL , PRIMARY KEY (`id_user`))');
    transaction.executeSql("INSERT INTO `user` (`id_user`, `nama_user`, `username`, `password`) VALUES (NULL, 'nurcahya pradana', 'admin', 'admin')");
  });
}
...
function showCount() {
  database.transaction(function(transaction) {
    transaction.executeSql('SELECT count(*) AS recordCount FROM user', [], function(ignored, resultSet) {
